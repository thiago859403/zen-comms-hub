...
export default function HttpNode({ data, selected }: NodeProps) {
  return (
    <div className={`px-4 py-3 rounded-lg bg-card border-2 border-border shadow-md transition-all min-w-[200px] ${selected ? 'ring-2 ring-ring ring-offset-2 border-indigo-500' : ''}`}>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="font-semibold text-sm text-foreground">HTTP Request</div>
      </div>
      
      <div className="text-xs text-muted-foreground line-clamp-2">
        {String(data.method || 'GET')} - {String(data.url || 'Configure URL')}
      </div>
